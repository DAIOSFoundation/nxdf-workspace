...
const SwapDetailScreen = ({ item }) => {
  ...
  return (
    <SafeAreaView bgNavyTheme>
      <Topbar isLeftButton title={`${item.symbol} Swap`} />
      <TopView>
        <FromView>
          <FromText>From</FromText>
          <FromText>{`Balance: ${item.balance} ETH`}</FromText>
        </FromView>
        <SwapBox>
          <SwapCoin>
            <Image source={item.logo} height={20} width={20}/>
            <Text ftWhite marginLeft={5}>{item.symbol}</Text>
          </SwapCoin>
          <SwapPrice>
            <SwapText>{`${item.balance} ETH`}</SwapText>
    ...
    {/* to */}
      <SwapView>
        ...
        <SwapBox>
          ...
          <SwapPrice>
            <ToSwapText>{`${item.balance} ETH`}</ToSwapText>
          </SwapPrice>
        </SwapBox>
        <ToText>{`Balance: ${item.balance} ETH`}</ToText>
      </SwapView>
      {/* 상세 정보 창 */}
      <View flex={3} marginTop={30}>
        <DesView height={150} width={'94%'} alignSelf={'center'} justifySelf={'center'} bgLightNavy>
          ...
          <ViewRow>
            ...
            <Text width={'50%'} ftWhite  textAlign={'right'}>{`${item.name} > ${item.name}`}</Text>
          </ViewRow>
        </DesView>
      </View>
      {/* 스위치 */}
